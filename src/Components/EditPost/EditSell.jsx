...
class EditSell extends Component {
  ...
  static getDerivedStateFromProps(props,state){
   ...
   validateField =  async (fieldName, value) => {
    ...
    switch (fieldName) {
      ...
      case "title":
        ...
        break;
      case "desc":
        value.length <= 5 || value.length > 250
          ?  val = false
          :  val = true
        break;
      default:
   ...
   await this.setState({fieldValid:{...this.state.fieldValid,[fieldName]:val}})
    this.validateForm();
  };
  ...
  validateForm = async () => {
    await this.setState({
     formValid: this.state.fieldValid["category"] &&
      ...
      this.state.fieldValid["brand"] &
      this.state.fieldValid["color"] &&
